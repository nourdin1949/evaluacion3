 ...
 */
public class Abstract_Ejercicio2_EmpSalariado extends Abstract_Ejercicio2_Empleado {

    public Abstract_Ejercicio2_EmpSalariado() {
    }

    public Abstract_Ejercicio2_EmpSalariado(String DNI, String nombre, String apellidos, int aniosExpe, double salario) {
        super(DNI, nombre, apellidos, aniosExpe, salario);

    }

    @Override
    public void calcularSalario() {
        if (super.getAniosExpe()>= 2 && super.getAniosExpe() <= 3) {

            setSalarioFijo(super.getSalarioFijo() * 1.02);

        } else if (super.getAniosExpe() >= 4 && super.getAniosExpe() <= 7) {

            setSalarioFijo(super.getSalarioFijo() * 1.05);

        } else if (super.getAniosExpe()>= 8 && super.getAniosExpe() <= 15) {

            setSalarioFijo(super.getSalarioFijo() * 1.10);

        } else if (super.getAniosExpe()> 15) {

            setSalarioFijo(super.getSalarioFijo() * 1.15);
        }
    }

    @Override
    public void mostrar() {
        System.out.println("\t\t\t\t\t\tEmpleado Salariado\n");
        super.mostrar();
